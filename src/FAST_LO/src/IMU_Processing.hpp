...
class ImuProcess {
   public:
    ...
    int lidar_type;
    bool imu_en;
    bool disable_motion_compensation;
...
void ImuProcess::set_mean_acc_norm(const double& mean_acc_norm) { IMU_mean_acc_norm = mean_acc_norm; }
...
void ImuProcess::propagation_and_undist(const MeasureGroup& meas, StatesGroup& state_inout, PointCloudXYZI& pcl_out) {
    ...
    if (lidar_type != L515) {
        ...
        cout << "[ IMU Process ]: vel " << state_inout.vel_end.transpose() << " pos " << state_inout.pos_end.transpose()
             << " ba" << state_inout.bias_a.transpose() << " bg " << state_inout.bias_g.transpose() << endl;
        ...
        if (!disable_motion_compensation) {
            ...
            for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--) {
                ...
                for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
                    ...
                    V3D P_compensate = state_inout.offset_R_L_I.transpose() *
                                       (state_inout.rot_end.transpose() *
                                            (R_i * (state_inout.offset_R_L_I * p_in + state_inout.offset_T_L_I) + P_i -
                                             state_inout.pos_end) -
                                        state_inout.offset_T_L_I);
