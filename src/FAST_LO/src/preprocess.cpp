...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(1.0), point_filter_num(1) {
    inf_bound = 10;
    N_SCANS = 6;
    group_size = 8;
    disA = 0.01;
...
void Preprocess::process_cut_frame_pcl2(const sensor_msgs::PointCloud2::ConstPtr& msg,
    ...
    if (lidar_type == VELO) {
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            if (dist < blind * blind || isnan(added_pt.x) || isnan(added_pt.y) || isnan(added_pt.z)) continue;
        ...
        // Detect whether the ring is 8bit or 16bit encoded
        bool is_8bit = false;
        auto it = msg->fields.begin();
        for (; it != msg->fields.end(); ++it) {
            if (it->name == "ring") {
                is_8bit = static_cast<int>(it->datatype) == 2;
                break;
            }
        }

        if (is_8bit) {
            pcl::PointCloud<ouster_ros_ring8bit::Point> pl_orig;
            pcl::fromROSMsg(*msg, pl_orig);
            int plsize = pl_orig.points.size();
            pl_surf.reserve(plsize);
            for (int i = 0; i < plsize; i++) {
...
void Preprocess::oust_handler(const sensor_msgs::PointCloud2::ConstPtr& msg) {
    ...
    if (feature_enabled) {
        ...
        for (int i = 0; i < pl_orig.points.size(); i++) {
            ...
            if (pl_orig.points[i].ring < N_SCANS) pl_surf.points.push_back(added_pt);
        }
    }
}
...
void Preprocess::velodyne_handler_kitti(const sensor_msgs::PointCloud2::ConstPtr& msg) {
    ...
    for (int i = 0; i < pl_full.size(); i++) {
        if (i % point_filter_num == 0) {
            ...
            if (dist > blind * blind) pl_surf.points.push_back(pl_full[i]);
        }
    }
}
