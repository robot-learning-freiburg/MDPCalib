...
void LI_Init::push_ALL_IMU_CalibState(const sensor_msgs::Imu::ConstPtr& msg, const double& mean_acc_norm) {
    ...
    IMUstate.linear_acc = V3D(msg->linear_acceleration.x, msg->linear_acceleration.y, msg->linear_acceleration.z) /
...
void LI_Init::downsample_interpolate_IMU(const double& move_start_time) {
    while (IMU_state_group_ALL.front().timeStamp < move_start_time - 3.0) IMU_state_group_ALL.pop_front();
    while (Lidar_state_group.front().timeStamp < move_start_time - 3.0) Lidar_state_group.pop_front();
        ...
        for (int j = 1; j < IMU_state_group_ALL.size(); j++) {
            if (IMU_state_group_ALL[j - 1].timeStamp <= Lidar_state_group[i].timeStamp &&
                ...
                IMU_state_interpolation.ang_vel =
                    s * IMU_state_group_ALL[j - 1].ang_vel + (1 - s) * IMU_state_group_ALL[j].ang_vel;
                IMU_state_interpolation.linear_acc =
                    s * IMU_state_group_ALL[j - 1].linear_acc + (1 - s) * IMU_state_group_ALL[j].linear_acc;
...
void LI_Init::central_diff() {
    ...
    for (; it_Lidar_state != Lidar_state_group.end() - 2; it_Lidar_state++) {
        ...
        fout_LiDAR_meas << setprecision(12) << it_Lidar_state->ang_vel.transpose() << " "
                        << it_Lidar_state->ang_vel.norm() << " " << (it_Lidar_state->linear_acc - STD_GRAV).transpose()
                        << " " << it_Lidar_state->ang_acc.transpose() << " " << it_Lidar_state->timeStamp << endl;
    }
}
...
void LI_Init::IMU_time_compensate(const double& lag_time, const bool& is_discard) {
    ...
    while (Lidar_state_group.front().timeStamp < IMU_state_group.front().timeStamp) Lidar_state_group.pop_front();
    while (Lidar_state_group.front().timeStamp > IMU_state_group[1].timeStamp) IMU_state_group.pop_front();
    ...
    for (int i = 1; i < Lidar_state_group.size() - 1; i++) {
        ...
        if (deltaT > 0) {
            ...
            IMU_state_group[i].linear_acc =
                s * IMU_state_group[i + 1].linear_acc + (1 - s) * IMU_state_group[i].linear_acc;
...
void LI_Init::solve_Rotation_only() {
    ...
    for (int i = 0; i < IMU_state_group.size(); i++) {
        ...
        problem_rot.AddResidualBlock(
            Angular_Vel_Cost_only_Rot::Create(IMU_state_group[i].ang_vel, Lidar_state_group[i].ang_vel), nullptr,
            R_LI_quat);
    }
...
void LI_Init::solve_Rot_bias_gyro(double& timediff_imu_wrt_lidar) {
    ...
    for (int i = 0; i < IMU_state_group.size(); i++) {
        ...
        problem_ang_vel.AddResidualBlock(
            Angular_Vel_Cost::Create(IMU_state_group[i].ang_vel, IMU_state_group[i].ang_acc,
...
void LI_Init::solve_trans_biasacc_grav() {
    ...
    for (int i = 0; i < IMU_state_group.size(); i++) {
        problem_acc.AddResidualBlock(
            Linear_acc_Cost::Create(Lidar_state_group[i], Rot_Lidar_wrt_IMU, IMU_state_group[i].linear_acc), nullptr,
            R_GL0_quat, bias_aL, Trans_IL);
...
bool LI_Init::data_sufficiency_assess(MatrixXd& Jacobian_rot, int& frame_num, V3D& lidar_omg, int& orig_odom_freq,
    ...
    if (frame_num % orig_odom_freq * cut_frame_num == 0) {
        ...
        V3D Rot_percent(Scaled_Eigen[1] * Scaled_Eigen[2], Scaled_Eigen[0] * Scaled_Eigen[2],
                        Scaled_Eigen[0] * Scaled_Eigen[1]);

        V3D Rot_percent_scaled(Rot_percent[0] < 0.99 ? Rot_percent[0] : 1, Rot_percent[1] < 0.99 ? Rot_percent[1] : 1,
...
void LI_Init::print_initialization_result(double& time_L_I, M3D& R_L_I, V3D& p_L_I, V3D& bias_g, V3D& bias_a,
                                          V3D gravity) {
    cout.setf(ios::fixed);
...
void LI_Init::plot_result() {
    ...
    for (auto it_Lidar_state = Lidar_state_group.begin(); it_Lidar_state != Lidar_state_group.end() - 1;
         it_Lidar_state++) {
        for (int i = 0; i < 3; i++) {
